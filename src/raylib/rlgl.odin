...
VertexBufferIndexType :: cushort when RL_GRAPHICS_API_OPENGL_ES2 else cuint

// Dynamic vertex buffers (position + texcoords + colors + indices arrays)
VertexBuffer :: struct {
	elementCount: cint,                 // Number of elements in the buffer (QUADS)
	...
	vboId:     [4]cuint,                // OpenGL Vertex Buffer Objects id (4 types of vertex data)
}
...
// RenderBatch type
RenderBatch :: struct {
	bufferCount:   cint,           // Number of vertex buffers (multi-buffering support)
	...
	drawCounter:   cint,           // Draw calls counter
...
ShaderAttributeDataType :: enum cint {
...
FramebufferAttachType :: enum cint {
...
FramebufferAttachTextureType :: enum cint {
...
foreign  {
	...
	//------------------------------------------------------------------------------------
	rlMatrixMode   :: proc(mode: cint) ---                 // Choose the current matrix to be transformed
	...
	rlViewport     :: proc(x, y, width, height: cint) ---  // Set the viewport area

	//------------------------------------------------------------------------------------
	// Functions Declaration - Vertex level operations
	//------------------------------------------------------------------------------------
	rlBegin        :: proc(mode: cint)     --- // Initialize drawing mode (how to organize vertex)
	rlEnd          :: proc()                --- // Finish vertex providing
	rlVertex2i     :: proc(x, y: cint)     --- // Define one vertex (position) - 2 int
	...
	rlEnableVertexArray          :: proc(vaoId: cuint) -> bool --- // Enable vertex array (VAO, if supported)
	rlDisableVertexArray         :: proc() ---                      // Disable vertex array (VAO, if supported)
	rlEnableVertexBuffer         :: proc(id: cuint) ---            // Enable vertex buffer (VBO)
	rlDisableVertexBuffer        :: proc() ---                      // Disable vertex buffer (VBO)
	rlEnableVertexBufferElement  :: proc(id: cuint) ---            // Enable vertex buffer element (VBO element)
	rlDisableVertexBufferElement :: proc() ---                      // Disable vertex buffer element (VBO element)
	rlEnableVertexAttribute      :: proc(index: cuint) ---         // Enable vertex attribute index
	rlDisableVertexAttribute     :: proc(index: cuint) ---         // Disable vertex attribute index
	when RL_GRAPHICS_API_OPENGL_11 {
		rlEnableStatePointer :: proc(vertexAttribType: cint, buffer: rawptr) ---
		rlDisableStatePointer :: proc(vertexAttribType: cint) ---
	}

	// Textures state
	rlActiveTextureSlot     :: proc(slot: cint) ---                            // Select and active a texture slot
	rlEnableTexture         :: proc(id: cuint) ---                             // Enable texture
	rlDisableTexture        :: proc() ---                                       // Disable texture
	rlEnableTextureCubemap  :: proc(id: cuint) ---                             // Enable texture cubemap
	rlDisableTextureCubemap :: proc() ---                                       // Disable texture cubemap
	rlTextureParameters     :: proc(id: cuint, param: cint, value: cint) --- // Set texture parameters (filter, wrap)
	rlCubemapParameters     :: proc(id: i32, param: cint, value: cint) ---    // Set cubemap parameters (filter, wrap)

	// Shader state
	rlEnableShader  :: proc(id: cuint) ---                                       // Enable shader program
	rlDisableShader :: proc() ---                                                 // Disable shader program

	// Framebuffer state
	rlEnableFramebuffer  :: proc(id: cuint) ---                                  // Enable render texture (fbo)
	rlDisableFramebuffer :: proc() ---                                            // Disable render texture (fbo), return to default framebuffer
	rlActiveDrawBuffers  :: proc(count: cint) ---                                // Activate multiple draw color buffers
	rlBlitFramebuffer	 :: proc(srcX, srcY, srcWidth, srcHeight, dstX, dstY, dstWidth, dstHeight, bufferMask: cint) --- // Blit active framebuffer to main framebuffer
	...
	rlScissor                :: proc(x, y, width, height: cint) --- // Scissor test
	...
	rlSetBlendMode            :: proc(mode: cint) ---                                                           // Set blending mode
	rlSetBlendFactors         :: proc(glSrcFactor, glDstFactor, glEquation: cint) ---                           // Set blending mode factor and equation (using OpenGL factors)
	rlSetBlendFactorsSeparate :: proc(glSrcRGB, glDstRGB, glSrcAlpha, glDstAlpha, glEqRGB, glEqAlpha: cint) --- // Set blending mode factors and equations separately (using OpenGL factors)

	//------------------------------------------------------------------------------------
	// Functions Declaration - rlgl functionality
	//------------------------------------------------------------------------------------
	// rlgl initialization functions
	rlglInit               :: proc(width, height: cint) --- // Initialize rlgl (buffers, shaders, textures, states)
	...
	rlSetFramebufferWidth  :: proc(width: cint) ---         // Set current framebuffer width
	rlGetFramebufferWidth  :: proc() -> cint ---            // Get default framebuffer width
	rlSetFramebufferHeight :: proc(height: cint) ---        // Set current framebuffer height
	rlGetFramebufferHeight :: proc() -> cint ---            // Get default framebuffer height


	rlGetTextureIdDefault  :: proc() -> cuint ---   // Get default texture id
	rlGetShaderIdDefault   :: proc() -> cuint ---   // Get default shader id
	rlGetShaderLocsDefault :: proc() -> [^]cint --- // Get default shader locations

	// Render batch management
	// NOTE: rlgl provides a default render batch to behave like OpenGL 1.1 immediate mode
	// but this render batch API is exposed in case of custom batches are required
	rlLoadRenderBatch       :: proc(numBuffers, bufferElements: cint) -> RenderBatch --- // Load a render batch system
	...
	rlCheckRenderBatchLimit :: proc(vCount: cint) -> cint ---                           // Check internal buffer overflow for a given number of vertex

	rlSetTexture :: proc(id: cuint) --- // Set current texture for render batch and check buffers limits

	//------------------------------------------------------------------------------------------------------------------------

	// Vertex buffers management
	rlLoadVertexArray                  :: proc() -> cuint ---                                                      // Load vertex array (vao) if supported
	rlLoadVertexBuffer                 :: proc(buffer: rawptr, size: cint, is_dynamic: bool) -> cuint ---         // Load a vertex buffer attribute
	rlLoadVertexBufferElement          :: proc(buffer: rawptr, size: cint, is_dynamic: bool) -> cuint ---         // Load a new attributes element buffer
	rlUpdateVertexBuffer               :: proc(bufferId: cuint, data: rawptr, dataSize: cint, offset: cint) ---  // Update GPU buffer with new data
	rlUpdateVertexBufferElements       :: proc(id: cuint, data: rawptr, dataSize: cint, offset: cint) ---        // Update vertex buffer elements with new data
	rlUnloadVertexArray                :: proc(vaoId: cuint) ---
	rlUnloadVertexBuffer               :: proc(vboId: cuint) ---
	rlSetVertexAttribute               :: proc(index: cuint, compSize: cint, type: cint, normalized: bool, stride: cint, pointer: rawptr) ---
	rlSetVertexAttributeDivisor        :: proc(index: cuint, divisor: cint) ---
	rlSetVertexAttributeDefault        :: proc(locIndex: cint, value: rawptr, attribType: cint, count: cint) --- // Set vertex attribute default value
	rlDrawVertexArray                  :: proc(offset: cint, count: cint) ---
	rlDrawVertexArrayElements          :: proc(offset: cint, count: cint, buffer: rawptr) ---
	rlDrawVertexArrayInstanced         :: proc(offset: cint, count: cint, instances: cint) ---
	rlDrawVertexArrayElementsInstanced :: proc(offset: cint, count: cint, buffer: rawptr, instances: cint) ---

	// Textures management
	rlLoadTexture         :: proc(data: rawptr, width, height: cint, format: cint, mipmapCount: cint) -> cuint ---        // Load texture in GPU
	rlLoadTextureDepth    :: proc(width, height: cint, useRenderBuffer: bool) -> cuint ---                                  // Load depth texture/renderbuffer (to be attached to fbo)
	rlLoadTextureCubemap  :: proc(data: rawptr, size: cint, format: cint) -> cuint ---                                     // Load texture cubemap
	rlUpdateTexture       :: proc(id: cuint, offsetX, offsetY: cint, width, height: cint, format: cint, data: rawptr) --- // Update GPU texture with new data
	rlGetGlTextureFormats :: proc(format: cint, glInternalFormat, glFormat, glType: ^cuint) ---                             // Get OpenGL internal formats
	rlGetPixelFormatName  :: proc(format: cuint) -> cstring ---                                                              // Get name string for pixel format
	rlUnloadTexture       :: proc(id: cuint) ---                                                                             // Unload texture from GPU memory
	rlGenTextureMipmaps   :: proc(id: cuint, width, height: cint, format: cint, mipmaps: ^cint) ---                       // Generate mipmap data for selected texture
	rlReadTexturePixels   :: proc(id: cuint, width, height: cint, format: cint) -> rawptr ---                              // Read texture pixel data
	rlReadScreenPixels    :: proc(width, height: cint) -> [^]byte ---                                                        // Read screen pixel data (color buffer)

	// Framebuffer management (fbo)
	rlLoadFramebuffer     :: proc(width, height: cint) -> cuint ---                                           // Load an empty framebuffer
	rlFramebufferAttach   :: proc(fboId, texId: cuint, attachType: cint, texType: cint, mipLevel: cint) --- // Attach texture/renderbuffer to a framebuffer
	rlFramebufferComplete :: proc(id: cuint) -> bool ---                                                       // Verify framebuffer is complete
	rlUnloadFramebuffer   :: proc(id: cuint) ---                                                               // Delete framebuffer from GPU

	// Shaders management
	rlLoadShaderCode      :: proc(vsCode, fsCode: cstring) -> cuint ---                                // Load shader from code strings
	rlCompileShader       :: proc(shaderCode: cstring, type: cint) -> cuint ---                       // Compile custom shader and return shader id (type: RL_VERTEX_SHADER, RL_FRAGMENT_SHADER, RL_COMPUTE_SHADER)
	rlLoadShaderProgram   :: proc(vShaderId, fShaderId: cuint) -> cuint ---                           // Load custom shader program
	rlUnloadShaderProgram :: proc(id: cuint) ---                                                       // Unload shader program
	rlGetLocationUniform  :: proc(shaderId: cuint, uniformName: cstring) -> cint ---                  // Get shader location uniform
	rlGetLocationAttrib   :: proc(shaderId: cuint, attribName: cstring) -> cint ---                   // Get shader location attribute
	rlSetUniform          :: proc(locIndex: cint, value: rawptr, uniformType: cint, count: cint) --- // Set shader value uniform
	rlSetUniformMatrix    :: proc(locIndex: cint, mat: Matrix) ---                                     // Set shader value matrix
	rlSetUniformSampler   :: proc(locIndex: cint, textureId: cuint) ---                               // Set shader value sampler
	rlSetShader           :: proc(id: cuint, locs: [^]cint) ---                                       // Set shader currently active (id and locations)

	// Compute shader management
	rlLoadComputeShaderProgram :: proc(shaderId: cuint) -> cuint ---     // Load compute shader program
	rlComputeShaderDispatch    :: proc(groupX, groupY, groupZ: cuint) --- // Dispatch compute shader (equivalent to *draw* for graphics pipeline)

	// Shader buffer storage object management (ssbo)
	rlLoadShaderBuffer    :: proc(size: cuint, data: rawptr, usageHint: cint) -> cuint ---              // Load shader storage buffer object (SSBO)
	rlUnloadShaderBuffer  :: proc(ssboId: cuint) ---                                                      // Unload shader storage buffer object (SSBO)
	rlUpdateShaderBuffer  :: proc(id: cuint, data: rawptr, dataSize: cuint, offset: cuint) ---          // Update SSBO buffer data
	rlBindShaderBuffer    :: proc(id: cuint, index: cuint) ---                                           // Bind SSBO buffer
	rlReadShaderBuffer    :: proc(id: cuint, dest: rawptr, count: cuint, offset: cuint) ---             // Read SSBO buffer data (GPU->CPU)
	rlCopyShaderBuffer    :: proc(destId, srcId: cuint, destOffset, srcOffset: cuint, count: cuint) --- // Copy SSBO data between buffers
	rlGetShaderBufferSize :: proc(id: cuint) -> cuint ---                                                // Get SSBO buffer size

	// Buffer management
	rlBindImageTexture :: proc(id: cuint, index: cuint, format: cint, readonly: bool) ---  // Bind image texture
	...
	rlGetMatrixProjectionStereo :: proc(eye: cint) -> Matrix --- // Get internal projection matrix for stereo render (selected eye)
	rlGetMatrixViewOffsetStereo :: proc(eye: cint) -> Matrix --- // Get internal view offset matrix for stereo render (selected eye)
