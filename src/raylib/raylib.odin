...
#assert(size_of(rune) == size_of(cint))
...
// File path list
FilePathList :: struct {
    capacity: cuint,                     // Filepaths max entries
    count:    cuint,                     // Filepaths entries count
    paths:    [^]cstring,                 // Filepaths entries
}

// Automation event
AutomationEvent :: struct {
	frame:      cuint,               // Event frame
	type:       cuint,               // Event type (AutomationEventType)
	params:     [4]cint,             // Event parameters (if required) ---
}

// Automation event list
AutomationEventList :: struct {
	capacity:   cuint,                // Events max entries (MAX_AUTOMATION_EVENTS)
	count:      cuint,                // Events entries count
...
ConfigFlags :: distinct bit_set[ConfigFlag; cint]


// Trace log level
TraceLogLevel :: enum cint {
...
ShaderUniformDataType :: enum cint {
...
Gestures :: distinct bit_set[Gesture; cuint]

// Camera system modes
CameraMode :: enum cint {
...
// Camera projection
CameraProjection :: enum cint {
	PERSPECTIVE = 0,                  // Perspective projection
	ORTHOGRAPHIC,                     // Orthographic projection
}

// N-patch layout
NPatchLayout :: enum cint {
...
LoadFileDataCallback :: #type proc "c"(fileName: cstring, dataSize: ^cint) -> [^]u8                  	// FileIO: Load binary data
SaveFileDataCallback :: #type proc "c" (fileName: cstring, data: rawptr, dataSize: cint) -> bool  		// FileIO: Save binary data
...
AudioCallback :: #type proc "c" (bufferData: rawptr, frames: cuint)
...
foreign  {
	...
	InitWindow               :: proc(width, height: cint, title: cstring) ---  // Initialize window and OpenGL context
	...
	SetWindowIcons           :: proc(images: [^]Image, count: cint) ---        // Set icon for window (multiple images, RGBA 32bit, only PLATFORM_DESKTOP)
	SetWindowTitle           :: proc(title: cstring) ---                        // Set title for window (only PLATFORM_DESKTOP and PLATFORM_WEB)
	SetWindowPosition        :: proc(x, y: cint) ---                           // Set window position on screen (only PLATFORM_DESKTOP)
	SetWindowMonitor         :: proc(monitor: cint) ---                        // Set monitor for the current window
	SetWindowMinSize         :: proc(width, height: cint) ---                  // Set window minimum dimensions (for FLAG_WINDOW_RESIZABLE)
	SetWindowMaxSize         :: proc(width, height: cint) ---                  // Set window maximum dimensions (for FLAG_WINDOW_RESIZABLE)
	SetWindowSize            :: proc(width, height: cint) ---                  // Set window dimensions
	...
	GetScreenWidth           :: proc() -> cint ---                             // Get current screen width
	GetScreenHeight          :: proc() -> cint ---                             // Get current screen height
	GetRenderWidth           :: proc() -> cint ---                             // Get current render width (it considers HiDPI)
	GetRenderHeight          :: proc() -> cint ---                             // Get current render height (it considers HiDPI)
	GetMonitorCount          :: proc() -> cint ---                             // Get number of connected monitors
	GetCurrentMonitor        :: proc() -> cint ---                             // Get current connected monitor
	GetMonitorPosition       :: proc(monitor: cint) -> Vector2 ---             // Get specified monitor position
	GetMonitorWidth          :: proc(monitor: cint) -> cint ---               // Get specified monitor width (current video mode used by monitor)
	GetMonitorHeight         :: proc(monitor: cint) -> cint ---               // Get specified monitor height (current video mode used by monitor)
	GetMonitorPhysicalWidth  :: proc(monitor: cint) -> cint ---               // Get specified monitor physical width in millimetres
	GetMonitorPhysicalHeight :: proc(monitor: cint) -> cint ---               // Get specified monitor physical height in millimetres
	GetMonitorRefreshRate    :: proc(monitor: cint) -> cint ---               // Get specified monitor refresh rate
	...
	GetMonitorName           :: proc(monitor: cint) -> cstring ---             // Get the human-readable, UTF-8 encoded name of the specified monitor
	...
	BeginScissorMode  :: proc(x, y, width, height: cint) --- // Begin scissor mode (define screen area for following drawing)
	...
	GetShaderLocation       :: proc(shader: Shader, uniformName: cstring) -> cint ---                                                    // Get shader uniform location
	GetShaderLocationAttrib :: proc(shader: Shader, attribName: cstring) -> cint ---                                                     // Get shader attribute location
	...
	SetShaderValueV         :: proc(shader: Shader, locIndex: ShaderLocationIndex, value: rawptr, uniformType: ShaderUniformDataType, count: cint) --- // Set shader uniform value vector
	...
	GetWorldToScreenEx :: proc(position: Vector3, camera: Camera, width, height: cint) -> Vector2 --- // Get size position for a 3d world space position
	...
	SetTargetFPS :: proc(fps: cint) --- // Set target FPS (maximum)
	GetFPS       :: proc() -> cint ---  // Returns current FPS
	...
	SetRandomSeed  		 :: proc(seed: cuint) ---                      // Set the seed for the random number generator
	GetRandomValue 		 :: proc(min, max: cint) -> cint ---          // Get a random value between min and max (both included)
	LoadRandomSequence 	 :: proc(count : cuint, min, max: cint) --- 	// Load random values sequence, no values repeated
	UnloadRandomSequence :: proc(sequence : ^cint) ---             	// Unload random values sequence
	...
	MemAlloc         :: proc(size: cuint) -> rawptr ---                                        // Internal memory allocator
	MemRealloc       :: proc(ptr: rawptr, size: cuint) -> rawptr ---                           // Internal memory reallocator
	...
	LoadFileData            :: proc(fileName: cstring, dataSize: ^cint) -> [^]byte ---                   // Load file data as byte array (read)
	UnloadFileData          :: proc(data: [^]byte) ---                                                    // Unload file data allocated by LoadFileData()
	SaveFileData            :: proc(fileName: cstring, data: rawptr, dataSize: cint) -> bool ---         // Save data to file from byte array (write), returns true on success
	ExportDataAsCode        :: proc(data: rawptr, dataSize: cint, fileName: cstring) -> bool ---         // Export data to code (.h), returns true on success
	...
	GetFileLength           :: proc(fileName: cstring) -> cint ---                                            // Get file length in bytes (NOTE: GetFileSize() conflicts with windows.h)
	...
	GetFileModTime          :: proc(fileName: cstring) -> clong ---                                           // Get file modification time (last write time)

	// Compression/Encoding functionality

	CompressData     :: proc(data: rawptr,     dataSize: cint,     compDataSize: ^cint) -> [^]byte ---       // Compress data (DEFLATE algorithm), memory must be MemFree()
	DecompressData   :: proc(compData: rawptr, compDataSize: cint, dataSize:     ^cint) -> [^]byte ---       // Decompress data (DEFLATE algorithm), memory must be MemFree()
	EncodeDataBase64 :: proc(data: rawptr,     dataSize: cint,     outputSize:   ^cint) -> [^]byte ---       // Encode data to Base64 string, memory must be MemFree()
	DecodeDataBase64 :: proc(data: rawptr,     outputSize: ^cint) -> [^]byte ---                              // Decode Base64 string data, memory must be MemFree()
	...
	SetAutomationEventBaseFrame   :: proc(frame: cint) ---                                         // Set automation event internal base frame to start recording
	...
	IsGamepadAvailable       :: proc(gamepad: cint) -> bool ---                        // Check if a gamepad is available
	GetGamepadName           :: proc(gamepad: cint) -> cstring ---                     // Get gamepad internal name id
	IsGamepadButtonPressed   :: proc(gamepad: cint, button: GamepadButton) -> bool --- // Check if a gamepad button has been pressed once
	IsGamepadButtonDown      :: proc(gamepad: cint, button: GamepadButton) -> bool --- // Check if a gamepad button is being pressed
	IsGamepadButtonReleased  :: proc(gamepad: cint, button: GamepadButton) -> bool --- // Check if a gamepad button has been released once
	IsGamepadButtonUp        :: proc(gamepad: cint, button: GamepadButton) -> bool --- // Check if a gamepad button is NOT being pressed
	GetGamepadButtonPressed  :: proc() -> GamepadButton ---                             // Get the last gamepad button pressed
	GetGamepadAxisCount      :: proc(gamepad: cint) -> cint ---                       // Get gamepad axis count for a gamepad
	GetGamepadAxisMovement   :: proc(gamepad: cint, axis: GamepadAxis) -> f32 ---      // Get axis movement value for a gamepad axis
	SetGamepadMappings       :: proc(mappings: cstring) -> cint ---                    // Set internal gamepad mappings (SDL_GameControllerDB)
	...
	GetMouseX             :: proc() -> cint ---                      // Returns mouse position X
	GetMouseY             :: proc() -> cint ---                      // Returns mouse position Y
	GetMousePosition      :: proc() -> Vector2 ---                    // Returns mouse position XY
	GetMouseDelta         :: proc() -> Vector2 ---                    // Returns mouse delta XY
	SetMousePosition      :: proc(x, y: cint) ---                    // Set mouse position XY
	SetMouseOffset        :: proc(offsetX, offsetY: cint) ---        // Set mouse offset
	...
	GetTouchX          :: proc() -> cint ---               // Returns touch position X for touch point 0 (relative to screen size)
	GetTouchY          :: proc() -> cint ---               // Returns touch position Y for touch point 0 (relative to screen size)
	GetTouchPosition   :: proc(index: cint) -> Vector2 --- // Returns touch position XY for a touch point index (relative to screen size)
	GetTouchPointId    :: proc(index: cint) -> cint ---   // Get touch point identifier for given index
	GetTouchPointCount :: proc() -> cint ---               // Get number of touch points
	...
	DrawPixel                 :: proc(posX, posY: cint, color: Color) ---                                                                          // Draw a pixel
	DrawPixelV                :: proc(position: Vector2, color: Color) ---                                                                          // Draw a pixel (Vector version)
	DrawLine                  :: proc(startPosX, startPosY, endPosX, endPosY: cint, color: Color) ---                                              // Draw a line
	...
	DrawLineStrip             :: proc(points: [^]Vector2, pointCount: cint, color: Color) ---                                                      // Draw lines sequence (using gl lines)
	DrawLineBezier            :: proc(startPos, endPos: Vector2, thick: f32, color: Color) ---                                                      // Draw line segment cubic-bezier in-out interpolation
	DrawCircle                :: proc(centerX, centerY: cint, radius: f32, color: Color) ---                                                       // Draw a color-filled circle
	DrawCircleSector          :: proc(center: Vector2, radius: f32, startAngle, endAngle: f32, segments: cint, color: Color) ---                   // Draw a piece of a circle
	DrawCircleSectorLines     :: proc(center: Vector2, radius: f32, startAngle, endAngle: f32, segments: cint, color: Color) ---                   // Draw circle sector outline
	DrawCircleGradient        :: proc(centerX, centerY: cint, radius: f32, color1, color2: Color) ---                                              // Draw a gradient-filled circle
	DrawCircleV               :: proc(center: Vector2, radius: f32, color: Color) ---                                                               // Draw a color-filled circle (Vector version)
	DrawCircleLines           :: proc(centerX, centerY: cint, radius: f32, color: Color) ---                                                       // Draw circle outline
	DrawCircleLinesV          :: proc(center: Vector2, radius: f32, color: Color) ---                                                               // Draw circle outline (Vector version)
	DrawEllipse               :: proc(centerX, centerY: cint, radiusH, radiusV: f32, color: Color) ---                                             // Draw ellipse
	DrawEllipseLines          :: proc(centerX, centerY: cint, radiusH, radiusV: f32, color: Color) ---                                             // Draw ellipse outline
	DrawRing                  :: proc(center: Vector2, innerRadius, outerRadius: f32, startAngle, endAngle: f32, segments: cint, color: Color) --- // Draw ring
	DrawRingLines             :: proc(center: Vector2, innerRadius, outerRadius: f32, startAngle, endAngle: f32, segments: cint, color: Color) --- // Draw ring outline
	DrawRectangle             :: proc(posX, posY: cint, width, height: cint, color: Color) ---                                                    // Draw a color-filled rectangle
	...
	DrawRectangleGradientV    :: proc(posX, posY: cint, width, height: cint, color1, color2: Color) ---                                           // Draw a vertical-gradient-filled rectangle
	DrawRectangleGradientH    :: proc(posX, posY: cint, width, height: cint, color1, color2: Color) ---                                           // Draw a horizontal-gradient-filled rectangle
	...
	DrawRectangleLines        :: proc(posX, posY: cint, width, height: cint, color: Color) ---                                                    // Draw rectangle outline
	DrawRectangleLinesEx      :: proc(rec: Rectangle, lineThick: f32, color: Color) ---                                                             // Draw rectangle outline with extended parameters
	DrawRectangleRounded      :: proc(rec: Rectangle, roundness: f32, segments: cint, color: Color) ---                                            // Draw rectangle with rounded edges
	DrawRectangleRoundedLines :: proc(rec: Rectangle, roundness: f32, segments: cint, lineThick: f32, color: Color) ---                            // Draw rectangle with rounded edges outline
	...
	DrawTriangleFan           :: proc(points: [^]Vector2, pointCount: cint, color: Color) ---                                                      // Draw a triangle fan defined by points (first vertex is the center)
	DrawTriangleStrip         :: proc(points: [^]Vector2, pointCount: cint, color: Color) ---                                                      // Draw a triangle strip defined by points
	DrawPoly                  :: proc(center: Vector2, sides: cint, radius: f32, rotation: f32, color: Color) ---                                  // Draw a regular polygon (Vector version)
	DrawPolyLines             :: proc(center: Vector2, sides: cint, radius: f32, rotation: f32, color: Color) ---                                  // Draw a polygon outline of n sides
	DrawPolyLinesEx           :: proc(center: Vector2, sides: cint, radius: f32, rotation: f32, lineThick: f32, color: Color) ---                  // Draw a polygon outline of n sides with extended parameters

	// Splines drawing functions
	DrawSplineLinear                 :: proc(points: [^]Vector2, pointCount: cint, thick: f32, color: Color) --- // Draw spline: Linear, minimum 2 points
	DrawSplineBasis                  :: proc(points: [^]Vector2, pointCount: cint, thick: f32, color: Color) --- // Draw spline: B-Spline, minimum 4 points
	DrawSplineCatmullRom             :: proc(points: [^]Vector2, pointCount: cint, thick: f32, color: Color) --- // Draw spline: Catmull-Rom, minimum 4 points
	DrawSplineBezierQuadratic        :: proc(points: [^]Vector2, pointCount: cint, thick: f32, color: Color) --- // Draw spline: Quadratic Bezier, minimum 3 points (1 control point): [p1, c2, p3, c4...]
	DrawSplineBezierCubic            :: proc(points: [^]Vector2, pointCount: cint, thick: f32, color: Color) --- // Draw spline: Cubic Bezier, minimum 4 points (2 control points): [p1, c2, c3, p4, c5, c6...]
	...
	CheckCollisionPointPoly     :: proc(point: Vector2, points: [^]Vector2, pointCount: cint) -> bool ---                                          // Check if point is within a polygon described by array of vertices
	...
	CheckCollisionPointLine     :: proc(point: Vector2, p1, p2: Vector2, threshold: cint) -> bool ---                                              // Check if point belongs to line created between two points [p1] and [p2] with defined margin in pixels [threshold]
	...
	LoadImageRaw         :: proc(fileName: cstring, width, height: cint, format: PixelFormat, headerSize: cint) -> Image --- // Load image from RAW file data
	LoadImageSvg         :: proc(fileNameOrString: cstring, width, height: cint) -> Image ---                                 // Load image from SVG file data or string with specified size
	LoadImageAnim        :: proc(fileName: cstring, frames: [^]cint) -> Image ---                                             // Load image sequence from file (frames appended to image.data)
	LoadImageFromMemory  :: proc(fileType: cstring, fileData: rawptr, dataSize: cint) -> Image ---                            // Load image from memory buffer, fileType refers to extension: i.e. '.png'
	...
	ExportImageToMemory  :: proc(image: Image, fileType: cstring, fileSize: ^cint) -> rawptr ---                              // Export image to memory buffer
	...
	GenImageColor          :: proc(width, height: cint, color: Color) -> Image  ---                                           // Generate image: plain color
	GenImageGradientLinear :: proc(width, height, direction: cint, start, end: Color) -> Image ---                            // Generate image: linear gradient, direction in degrees [0..360], 0=Vertical gradient
	GenImageGradientRadial :: proc(width, height: cint, density: f32, inner, outer: Color) -> Image  ---                      // Generate image: radial gradient
	GenImageGradientSquare :: proc(width, height: cint, density: f32, inner, outer: Color) -> Image  ---                      // Generate image: square gradient
	GenImageChecked        :: proc(width, height: cint, checksX, checksY: cint, col1, col2: Color) -> Image  ---             // Generate image: checked
	GenImageWhiteNoise     :: proc(width, height: cint, factor: f32) -> Image  ---                                            // Generate image: white noise
	GenImagePerlinNoise    :: proc(width, height: cint, offsetX, offsetY: cint, scale: f32) -> Image  ---                    // Generate image: perlin noise
	GenImageCellular       :: proc(width, height: cint, tileSize: cint) -> Image  ---                                        // Generate image: cellular algorithm, bigger tileSize means bigger cells
	GenImageText           :: proc(width, height: cint, text: cstring) -> Image  ---                                          // Generate image: grayscale image from text data
	...
	ImageText             :: proc(text: cstring, fontSize: cint, color: Color) -> Image ---                                   // Create an image from text (default font)
	...
	ImageBlurGaussian     :: proc(image: ^Image, blurSize: cint) ---                                                          // Apply Gaussian blur using a box blur approximation
	ImageResize           :: proc(image: ^Image, newWidth,  newHeight: cint) ---                                              // Resize image (Bicubic scaling algorithm)
	ImageResizeNN         :: proc(image: ^Image, newWidth,  newHeight: cint) ---                                              // Resize image (Nearest-Neighbor scaling algorithm)
	ImageResizeCanvas     :: proc(image: ^Image, newWidth,  newHeight: cint, offsetX, offsetY: cint, fill: Color) ---        // Resize canvas and fill with color
	ImageMipmaps          :: proc(image: ^Image) ---                                                                           // Compute all mipmap levels for a provided image
	ImageDither           :: proc(image: ^Image, rBpp, gBpp, bBpp, aBpp: cint) ---                                            // Dither image data to 16bpp or lower (Floyd-Steinberg dithering)
	ImageFlipVertical     :: proc(image: ^Image) ---                                                                           // Flip image vertically
	ImageFlipHorizontal   :: proc(image: ^Image) ---                                                                           // Flip image horizontally
	ImageRotate           :: proc(image: ^Image, degrees: cint) ---                                                           // Rotate image by input angle in degrees( -359 to 359)
	...
	ImageColorBrightness  :: proc(image: ^Image, brightness: cint) ---                                                        // Modify image color: brightness (-255 to 255)
	...
	LoadImagePalette      :: proc(image: Image, maxPaletteSize: cint, colorCount: ^cint) -> [^]Color ---                     // Load colors palette from image as a Color array (RGBA - 32bit)
	...
	GetImageColor         :: proc(image: Image, x, y: cint) -> Color ---                                                      // Get image pixel color at (x, y) position

	// Image drawing functions
	// NOTE: Image software-rendering functions (CPU)

	ImageClearBackground    :: proc(dst: ^Image, color: Color) ---                                                                           // Clear image background with given color
	ImageDrawPixel          :: proc(dst: ^Image, posX, posY: cint, color: Color) ---                                                        // Draw pixel within an image
	ImageDrawPixelV         :: proc(dst: ^Image, position: Vector2, color: Color) ---                                                        // Draw pixel within an image (Vector version)
	ImageDrawLine           :: proc(dst: ^Image, startPosX, startPosY, endPosX, endPosY: cint, color: Color) ---                            // Draw line within an image
	ImageDrawLineV          :: proc(dst: ^Image, start, end: Vector2, color: Color) ---                                                      // Draw line within an image (Vector version)
	ImageDrawCircle         :: proc(dst: ^Image, centerX, centerY: cint, radius: cint, color: Color) ---                                   // Draw a filled circle within an image
	ImageDrawCircleV        :: proc(dst: ^Image, center: Vector2, radius: cint, color: Color) ---                                           // Draw a filled circle within an image (Vector version)
	ImageDrawCircleLines    :: proc(dst: ^Image, centerX, centerY: cint, radius: cint, color: Color) ---                                   // Draw circle outline within an image
	ImageDrawCircleLinesV   :: proc(dst: ^Image, center: Vector2, radius: cint, color: Color) ---                                           // Draw circle outline within an image (Vector version)
	ImageDrawRectangle      :: proc(dst: ^Image, posX, posY: cint, width, height: cint, color: Color) ---                                  // Draw rectangle within an image
	...
	ImageDrawRectangleLines :: proc(dst: ^Image, rec: Rectangle, thick: cint, color: Color) ---                                             // Draw rectangle lines within an image
	...
	ImageDrawText           :: proc(dst: ^Image, text: cstring, posX, posY: cint, fontSize: cint, color: Color) ---                        // Draw text (using default font) within an image (destination)
	...
	LoadRenderTexture    :: proc(width, height: cint) -> RenderTexture2D ---               // Load texture for rendering (framebuffer)
	...
	DrawTexture       :: proc(texture: Texture2D, posX, posY: cint, tint: Color) ---                                                       // Draw a Texture2D
	...
	ColorToInt          :: proc(color: Color) -> cuint ---                             // Get hexadecimal value for a Color
	...
	GetColor            :: proc(hexValue: cuint) -> Color ---                          // Get Color structure from hexadecimal value
	...
	GetPixelDataSize    :: proc(width, height: cint, format: PixelFormat) -> cint --- // Get pixel data size in bytes for certain format
	...
	LoadFontEx         :: proc(fileName: cstring, fontSize: cint, codepoints: [^]rune, codepointCount: cint) -> Font ---                                                     // Load font from file with extended parameters, use NULL for codepoints and 0 for codepointCount to load the default character set
	LoadFontFromImage  :: proc(image: Image, key: Color, firstChar: rune) -> Font ---                                                                         		   // Load font from Image (XNA style)
	LoadFontFromMemory :: proc(fileType: cstring, fileData: rawptr, dataSize: cint, fontSize: cint, codepoints: [^]rune, codepointCount: cint) -> Font ---      	           // Load font from memory buffer, fileType refers to extension: i.e. '.ttf'
	IsFontReady        :: proc(font: Font) -> bool ---                                                                                                        		   // Check if a font is ready
	LoadFontData       :: proc(fileData: rawptr, dataSize: cint, fontSize: cint, codepoints: [^]rune, codepointCount: cint, type: FontType) -> [^]GlyphInfo --- 	           // Load font data for further use
	GenImageFontAtlas  :: proc(glyphs: [^]GlyphInfo, glyphRecs: ^[^]Rectangle, codepointCount: cint, fontSize: cint, padding: cint, packMethod: cint) -> Image ---         // Generate image font atlas using chars info
	UnloadFontData     :: proc(glyphs: [^]GlyphInfo, glyphCount: cint) ---                                                                                    		   // Unload font chars info data (RAM)
	...
	DrawFPS            :: proc(posX, posY: cint) ---                                                                                             		// Draw current FPS
	DrawText           :: proc(text: cstring, posX, posY: cint, fontSize: cint, color: Color) ---                                               		// Draw text (using default font)
	...
	DrawTextCodepoints :: proc(font: Font, codepoints: [^]rune, codepointCount: cint, position: Vector2, fontSize: f32, spacing: f32, tint: Color) ---     // Draw multiple character (codepoint)

	// Text font info functions

	SetTextLineSpacing :: proc(spacing: cint) ---                                                      // Set vertical line spacing when drawing with line-breaks
	MeasureText        :: proc(text: cstring, fontSize: cint) -> cint ---                             // Measure string width for default font
	MeasureTextEx      :: proc(font: Font, text: cstring, fontSize: f32, spacing: f32) -> Vector2 ---   // Measure string size for Font
	GetGlyphIndex      :: proc(font: Font, codepoint: rune) -> cint ---                                // Get glyph index position in font for a codepoint (unicode character), fallback to '?' if not found
	...
	LoadUTF8             :: proc(codepoints: [^]rune, length: cint) -> [^]byte --- // Load UTF-8 text encoded from codepoints array
	UnloadUTF8           :: proc(text: [^]byte) ---                                 // Unload UTF-8 text encoded from codepoints array
	LoadCodepoints       :: proc(text: rawptr, count: ^cint) -> [^]rune ---        // Load all codepoints from a UTF-8 text string, codepoints count returned by parameter
	UnloadCodepoints     :: proc(codepoints: [^]rune) ---                           // Unload codepoints data from memory
	GetCodepointCount    :: proc(text : cstring) -> cint ---                       // Get total number of codepoints in a UTF-8 encoded string
	GetCodepoint         :: proc(text: cstring, codepointSize: ^cint) -> rune ---  // Get next codepoint in a UTF-8 encoded string, 0x3f('?') is returned on failure
	GetCodepointNext     :: proc(text: cstring, codepointSize: ^cint) -> rune ---  // Get next codepoint in a UTF-8 encoded string, 0x3f('?') is returned on failure
	GetCodepointPrevious :: proc(text: cstring, codepointSize: ^cint) -> rune ---  // Get previous codepoint in a UTF-8 encoded string, 0x3f('?') is returned on failure
	CodepointToUTF8      :: proc(codepoint: rune, utf8Size: ^cint) -> cstring ---  // Encode one codepoint into UTF-8 byte array (array length returned as parameter)

	// Text strings management functions (no UTF-8 strings, only byte chars)
	// NOTE: Some strings allocate memory internally for returned strings, just be careful!

	TextCopy      :: proc(dst: [^]byte, src: cstring) -> cint ---                               // Copy one string to another, returns bytes copied
	TextIsEqual   :: proc(text1, text2: cstring) -> bool ---                                     // Check if two text string are equal
	TextLength    :: proc(text: cstring) -> cuint ---                                           // Get text length, checks for '\0' ending

	// TextFormat is defined at the bottom of this file

	TextSubtext   :: proc(text: cstring, position: cint, length: cint) -> cstring ---          // Get a piece of a text string
	TextReplace   :: proc(text: [^]byte, replace, by: cstring) -> [^]byte ---                    // Replace text string (WARNING: memory must be freed!)
	TextInsert    :: proc(text, insert: cstring, position: cint) -> [^]byte ---                 // Insert text in a position (WARNING: memory must be freed!)
	TextJoin      :: proc(textList: [^]cstring, count: cint, delimiter: cstring) -> cstring --- // Join text strings with delimiter
	TextSplit     :: proc(text: cstring, delimiter: byte, count: ^cint) -> [^]cstring ---       // Split text into multiple strings
	TextAppend    :: proc(text: [^]byte, append: cstring, position: ^cint) ---                  // Append text at specific position and move cursor!
	TextFindIndex :: proc(text, find: cstring) -> cint ---                                      // Find first text occurrence within a string
	...
	TextToInteger :: proc(text: cstring) -> cint ---                                            // Get integer value from text (negative values not supported)
	...
	DrawTriangleStrip3D  :: proc(points: [^]Vector3, pointCount: cint, color: Color) ---                                       // Draw a triangle strip defined by points
	...
	DrawSphereEx         :: proc(centerPos: Vector3, radius: f32, rings, slices: cint, color: Color) ---                       // Draw sphere with extended parameters
	DrawSphereWires      :: proc(centerPos: Vector3, radius: f32, rings, slices: cint, color: Color) ---                       // Draw sphere wires
	DrawCylinder         :: proc(position: Vector3, radiusTop, radiusBottom: f32, height: f32, slices: cint, color: Color) --- // Draw a cylinder/cone
	DrawCylinderEx       :: proc(startPos, endPos: Vector3, startRadius, endRadius: f32, sides: cint, color: Color) ---        // Draw a cylinder with base at startPos and top at endPos
	DrawCylinderWires    :: proc(position: Vector3, radiusTop, radiusBottom, height: f32, slices: cint, color: Color) ---      // Draw a cylinder/cone wires
	DrawCylinderWiresEx  :: proc(startPos, endPos: Vector3, startRadius, endRadius: f32, sides: cint, color: Color) ---        // Draw a cylinder wires with base at startPos and top at endPos
	DrawCapsule          :: proc(startPos, endPos: Vector3, radius: f32, slices, rings: cint, color: Color) ---                // Draw a capsule with the center of its sphere caps at startPos and endPos
	DrawCapsuleWires     :: proc(startPos, endPos: Vector3, radius: f32, slices, rings: cint, color: Color) ---                // Draw capsule wireframe with the center of its sphere caps at startPos and endPos
	...
	DrawGrid             :: proc(slices: cint, spacing: f32) ---                                                               // Draw a grid (centered at (0, 0, 0))
	...
	UpdateMeshBuffer   :: proc(mesh: Mesh, index: cint, data: rawptr, dataSize: cint, offset: cint) ---    // Update mesh vertex data in GPU for a specific buffer index
	...
	DrawMeshInstanced  :: proc(mesh: Mesh, material: Material, transforms: [^]Matrix, instances: cint) ---   // Draw multiple mesh instances with material and different transforms
	...
	GenMeshPoly       :: proc(sides: cint, radius: f32) -> Mesh ---               // Generate polygonal mesh
	GenMeshPlane      :: proc(width, lengthL: f32, resX, resZ: cint) -> Mesh ---  // Generate plane mesh (with subdivisions)
	GenMeshCube       :: proc(width, height, length: f32) -> Mesh ---              // Generate cuboid mesh
	GenMeshSphere     :: proc(radius: f32, rings, slices: cint) -> Mesh ---       // Generate sphere mesh (standard sphere)
	GenMeshHemiSphere :: proc(radius: f32, rings, slices: cint) -> Mesh ---       // Generate half-sphere mesh (no bottom cap)
	GenMeshCylinder   :: proc(radius, height: f32, slices: cint) -> Mesh ---      // Generate cylinder mesh
	GenMeshCone       :: proc(radius, height: f32, slices: cint) -> Mesh ---      // Generate cone/pyramid mesh
	GenMeshTorus      :: proc(radius, size: f32, radSeg, sides: cint) -> Mesh --- // Generate torus mesh
	GenMeshKnot       :: proc(radius, size: f32, radSeg, sides: cint) -> Mesh --- // Generate trefoil knot mesh
	...
	LoadMaterials         :: proc(fileName: cstring, materialCount: ^cint) -> [^]Material ---            // Load materials from model file
	...
	SetModelMeshMaterial  :: proc(model: ^Model, meshId: cint, materialId: cint) ---                    // Set material for a mesh

	// Model animations loading/unloading functions

	LoadModelAnimations   :: proc(fileName: cstring, animCount: ^cint) -> [^]ModelAnimation ---  // Load model animations from file
	UpdateModelAnimation  :: proc(model: Model, anim: ModelAnimation, frame: cint) ---           // Update model animation pose
	UnloadModelAnimation  :: proc(anim: ModelAnimation) ---                                       // Unload animation data
	UnloadModelAnimations :: proc(animations: [^]ModelAnimation, animCount: cint) ---            // Unload animation array data
	...
	LoadWaveFromMemory :: proc(fileType: cstring, fileData: rawptr, dataSize: cint) -> Wave --- // Load wave from memory buffer, fileType refers to extension: i.e. '.wav'
	...
	UpdateSound        :: proc(sound: Sound, data: rawptr, frameCount: cint) ---                // Update sound buffer with new data
	...
	WaveCrop          :: proc(wave: ^Wave, initSample, finalSample: cint) ---                 // Crop a wave to defined samples range
	WaveFormat        :: proc(wave: ^Wave, sampleRate, sampleSize: cint, channels: cint) --- // Convert wave data to desired format
	...
	LoadMusicStreamFromMemory :: proc(fileType: cstring, data: rawptr, dataSize: cint) -> Music --- // Load music stream from data
	...
	LoadAudioStream                 :: proc(sampleRate, sampleSize: cuint, channels: cuint) -> AudioStream --- // Load audio stream (to stream raw audio pcm data)
	...
	UpdateAudioStream               :: proc(stream: AudioStream, data: rawptr, frameCount: cint) ---            // Update audio stream buffers with data
	...
	SetAudioStreamBufferSizeDefault :: proc(size: cint) ---                                                     // Default size for new audio streams
...
MemAllocatorProc :: proc(allocator_data: rawptr, mode: mem.Allocator_Mode,
	...
	case .Resize, .Resize_Non_Zeroed:
		ptr := MemRealloc(old_memory, cuint(size))
		if ptr == nil {
			err = .Out_Of_Memory
			return
		}
